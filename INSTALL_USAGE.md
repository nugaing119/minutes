...
watcher는 `.mp4`, `.mkv`, `.mov` 파일을 감지한다. 파일 크기와 수정 시간이 일정 시간 변하지 않으면 녹화 저장이 끝난 것으로 보고 처리한다.
...
API provider를 바로 쓰지 않고 Codex가 회의록을 작성하게 할 때는 다음처럼 실행한다.
...
원본 영상, 추출 음성, 전사 파일, OCR 이미지, snapshot, 최종 회의록은 기본적으로 로컬 `~/minutes` 아래에 저장된다.
...
`.env`, `.venv/`, `.omx/`, job/output 산출물, 영상/음성/자막/DOCX 파일은 `.gitignore`로 제외된다. API 키는 GitHub에 올리지 말고 환경변수, `.env`, 또는 macOS Keychain을 사용한다.
