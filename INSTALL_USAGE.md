...
콘텐츠 모델 턴은 freeze 결과와 별개로 해시 체크포인트에 고정된다. sidecar 구조 오류가 있으면
근거를 다시 읽지 않는 repair 세션 하나만 선택적으로 실행되며, 실패 시 전체 콘텐츠를 자동으로
재실행하지 않는다.
...
실제 회의는 받아쓰기 대신 안건·핵심 논의·결정/합의·담당자/기한·후속조치·미해결 항목·위험 중심으로 작성한다. 한국어 회의록은 `~함`, `~하기로 함`, `~예정임`, `~필요함` 계열의 객관적 보고체를 일관되게 사용한다. 회의가 아닌 기술 분석·데모·교육 문서는 실제 문서 목적에 맞는 전문 문체를 선택한다. 최종 MD/DOCX에는 STT/OCR/Snapshot 원시 참조, skill/model/token, 전처리·렌더·QA 기록, 내부 파일명·경로·해시를 넣지 않으며 이 정보는 job 내부 파일에만 남긴다.
...
공식 확인 모드가 켜진 최종 문서는 `추가 검증이 필요한 항목`과 `외부 근거 확인`을 마지막 두 H2로 항상 둔다. 항목이 없으면 섹션을 없애지 않고 구체적인 사유와 확인일을 적는다. `외부 근거 확인`에는 영상 우선 원칙과 녹화 원문·개인·내부 식별정보 비전송 사실만 표시한다. 공식 문서를 사용했다면 `녹화 내용 보강 근거`와 `영상 내용과 상충하는 근거`를 구분하고 조사 목적, 결과, 확인일과 링크를 남긴다. STT/OCR/skill 처리 과정은 표시하지 않으며, 상충하더라도 영상 본문은 그대로 유지하고 그 뒤에 다른 H2를 추가하지 않는다.
...
콘텐츠 모델 턴이 정상 종료되면 `content_generation_checkpoint.json`이 prompt와 현재 파생
산출물 해시를 먼저 저장한다. freeze 실패 시 원시 근거 접근이 차단된 `content_repair` 세션이
허용된 sidecar 필드와 정확히 한 번 일치하는 Markdown 치환만 담은 작은 patch를 작성한다.
`apply_content_repair_patch.py`가 전체 patch를 선검증한 뒤 반영하고 freeze를 한 번만 재실행한다.
복구 실패 후에는 자동 전체 재생성을 거부한다. 실제로 근거를 처음부터 다시 읽어야 할 때만 다음처럼
명시한다.

```bash
./scripts/run_fresh_codex_job.py "<job-directory>" --force-content-rebuild
```
...
content 50회, 선택적 repair 12회, delivery 18회는 도구 왕복 비용 목표이며 감사나 전 페이지 검증을 줄이는 상한이 아니다.
...
content와 delivery의 품질 계약은 launcher가 각각 12KB·8KB 미만 prompt에 미리 넣고,
선택적 repair prompt는 최악의 bounded 오류를 포함해 11KB 미만으로 제한한다. worker는
