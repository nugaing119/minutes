...
Codex에 번들된 `$documents` skill의 렌더·전 페이지 검증 계약을 delivery 세션이 호출한다.
따라서 이 저장소에서는 `minutes` skill만 복사하며 `documents`를 별도로 vendor하거나
설치하지 않는다. 현재 Codex 설치에 `$documents`가 없으면 Codex를 먼저 업데이트해야 한다.
