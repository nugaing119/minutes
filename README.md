...
콘텐츠 모델 턴이 정상 종료되면 freeze 성공 여부와 무관하게 prompt·산출물 해시를
`content_generation_checkpoint.json`에 먼저 고정한다. sidecar schema나 정확 참조 때문에 freeze가
실패해도 전체 STT·OCR을 다시 읽지 않고, 원시 근거 접근이 차단된 repair 세션이 작은
`content_repair_patch.json`만 한 번 작성한다. 결정적 helper가 허용 필드와 정확 치환을 모두 검증한 뒤
반영하고 freeze를 한 번 재실행한다. 이 복구도 실패하면 자동 전체 재생성을 중단하며, 명시적인
`--force-content-rebuild`를 줘야만 파생 콘텐츠를 버리고 근거를 다시 읽는다.
...
근거를 다시 읽지 않는다. freeze 전 구조 복구가 필요한 경우에만 최대 1개의 격리된 repair 세션이
추가되며, 이 세션은 media/STT/OCR/evidence chunk/Snapshot을 읽을 수 없다. 정상 미디어 작업에서는 저장소 전체 회귀 테스트나 검증기 소스
...
절대 `.venv/bin/python` 경로로 freeze를 한 번 실행한다. content는 50회, repair는 12회, delivery는 18회 이하의
...
실제 회의는 대화 내용을 받아쓰지 않고 안건·핵심 논의·결정/합의·담당자/기한·후속조치·미해결 항목·위험을 중심으로 정리한다. 한국어 회의록은 `~함`, `~하기로 함`, `~예정임`, `~필요함` 계열의 간결하고 객관적인 보고체를 일관되게 사용한다. 기술 분석, 데모 해설, 교육 자료 등 회의가 아닌 문서는 해당 문서의 목적에 맞는 전문 문체를 사용하며 회의록 종결형을 강제하지 않는다.

완성본은 독자를 위한 결과물이다. STT/OCR/Snapshot 원시 참조, skill·model·token 정보, 전처리 단계, job 경로, 내부 JSON, 해시, 렌더 횟수와 QA 동작은 최종 MD/DOCX에 표시하지 않고 job 내부 감사·성능 파일에만 보존한다. 표지 메타데이터도 일시·소요 시간·목적·근거로 확인된 참석자처럼 독자에게 필요한 정보만 사용한다.
...
공식 확인 모드가 켜진 최종 문서는 마지막 두 H2를 `추가 검증이 필요한 항목`, `외부 근거 확인` 순서로 항상 둔다. 앞의 주제 수에 따라 Word 번호만 자동으로 이어지므로 6·7 같은 번호를 제목에 고정하지 않는다. 미해결 항목이나 외부 확인 대상이 없어도 섹션을 삭제하지 않고 판단 사유와 확인일을 표시한다. `외부 근거 확인`에는 영상 우선 원칙과 녹화 원문·개인·내부 식별정보를 외부 검색에 전송하지 않았다는 사실만 독자 관점에서 남긴다. 공식 문서를 사용했다면 `녹화 내용 보강 근거`와 `영상 내용과 상충하는 근거`를 구분하고 조사 목적, 공식 확인 결과, 차이 또는 보강 표현, 확인일과 링크를 기록한다. STT/OCR/skill 처리 과정은 표시하지 않으며 그 뒤에는 다른 H2를 두지 않는다.
...
content와 delivery worker에는 필요한 품질 규칙을 각각 12KB·8KB 미만의 compact prompt로 미리 넣고,
선택적 repair prompt는 최악의 bounded 오류를 포함해 11KB 미만으로 제한한다.
