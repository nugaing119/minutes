...
`Codex 모드`는 오디오 추출·전사·OCR을 로컬 처리한 뒤, 생성된 `codex_minutes_input.md`와 선별 Snapshot을 Codex가 읽어 근거 기반 화자 판단, 콘텐츠 유형, 제목, 섹션을 결정하고 목표 언어의 `minutes.md`를 작성한 다음 최종 output 폴더로 정리하는 방식이다.
