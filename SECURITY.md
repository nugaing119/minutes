...
reference를 다시 열지 못하게 한다. launcher는 command/read output을 model-visible output으로
계측한다. 한 결과가 24KB를 넘거나 금지한 지침 파일 읽기가 감지되면 해당 Codex process를 즉시
종료하고 위반 명령의 제한된 preview와 SHA-256만 job-local handoff에 남긴다. file-change diff는
별도 artifact-change 지표로 계측하지만 24KB 종료 상한에 포함하지 않는다. 따라서 문서 본문이나
DOCX 파일 크기를 제한하지 않는다.
24KB를 넘은 command/read 결과를 잘라 모델에 제공하거나 작업을 계속하지 않는다. evidence chunk는 part 전체를
