...
결과와 Snapshot 번호가 완료 순서에 따라 달라지지 않게 한다.
...
동결된 본문만 `standard_business_brief` preset과 `editorial_cover` header 규칙으로 만들고,
`finalize_docx.py`에서 `minutes.draft.docx`를 최종 편집한 `minutes.final.docx`로 확정한다.
...
2026-07-15 동일 31분 12.57초 영상의 acceptance 실행은 다음을 통과했다.
