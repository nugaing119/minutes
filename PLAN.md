...
회의, 강의, 웨비나 등 녹화 영상 파일을 Mac에서 로컬 처리해 전사 파일, OCR 보조 자료, 한국어 회의록 Markdown, DOCX를 생성한다.

기본 입력 폴더는 `~/remind/`다. 자동 감시 모드에서는 이 폴더에 새로 저장된 영상 파일을 감지한다. 수동 실행 모드에서는 영상 파일 경로를 직접 지정하며, 이 경우 파일이 꼭 `~/remind/` 안에 있을 필요는 없다. 시스템은 파일 저장 완료 또는 지정된 입력 파일을 기준으로 오디오 추출, STT, OCR, 회의록 생성, 최종 파일 정리를 순서대로 수행한다.
...
프로젝트 코드는 git clone 받은 위치에 둔다. 자동 처리할 영상 파일은 기본적으로 사용자 홈 디렉터리의 `remind/`에 저장한다. 내부 작업 파일과 결과물은 repo 내부가 아니라 사용자 홈 디렉터리 아래 `minutes/`에 저장한다.
...
영상 파일은 저장 중에도 파일이 먼저 보일 수 있으므로, 생성 이벤트만 보고 바로 처리하지 않는다. 파일 크기와 수정 시각이 일정 시간 동안 변하지 않을 때 저장 완료로 판단한다.
...
```text
새 영상 파일이 들어오면 자동으로 output 날짜 폴더까지 생성됨
```
